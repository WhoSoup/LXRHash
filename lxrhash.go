...
func (lx *LXRHash) HashSaver(src []byte) []byte {
	...
	B := func(v uint64) uint64 { lx.Accessed = append(lx.Accessed, v&mk); return uint64(lx.ByteMap[v&mk]) }
	b := func(v uint64) byte { return byte(B(v)) }
	...
	for i := 0; i < len(src); i++ {
		swp := (i+1)*int(src[(i+1)%len(src)]) + (len(src)-1-i)*int(src[(len(src)-1-i)%len(src)])
		swp = swp % len(src)
...
func (lx *LXRHash) HashPseudo(src []byte) []byte {
